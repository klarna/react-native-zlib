...
export default class App extends Component<Props> {
  onCompressApiClicked = () => {
    const testString = `Something to test`
    const testArray = [0,0,0,0,0,0,0,1,2,3,4,5,6]

    RNReactNativeZlib.deflate(testArray)
      .then(data => {
        RNReactNativeZlib.inflate(data).then(result => {
          if (result !== testArray)
            throw new Error(`Compression does not work`)
        })
      })
  ...
  render() {
    return (
      <View style={styles.container}>
        ...
        <Button onPress={this.onCompressApiClicked()}
                title="Press To Test"
        />
      </View>
    );
